...
#define FLASHCACHE_SW_VERSION "flashcache-1.0.cern.1"
...
flashcache_io_callback(unsigned long error, void *context)
{
	...
	if (unlikely(error)) {
		...
		if (! dmc->bypass) {  /* suppress massive console output */
#endif
	 ...
	 * cacheblock fails (b) we'd need to handle ssd metadata write failures
	 * as well and fall back to disk in those cases as well.
	 */
...
flashcache_do_pending_error(struct kcached_job *job)
{
	...
	cacheblk->cache_state &= ~(BLOCK_IO_INPROG);
	/*
	 * In case of an error in writeback or writearound modes, if there are pending
	 * jobs, de-link them from the cacheblock so we can issue disk IOs below.
	 */
	if (bio != NULL) {
	  VERIFY(dmc->cache_mode != FLASHCACHE_WRITE_BACK);
	...
	if (bio != NULL) {
	  ...
	  if (! dmc->bypass)  /* suppress massive console output */
#endif
	    DMERR("flashcache_do_pending_error: Re-launching errored IO to disk, after io error %d block %lu",
		  error, bio->bi_sector);
